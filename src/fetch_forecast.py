...
def main():
    ...
    try:
        ...
        # 4) Build and send reordered HTML summary with timestamp
        cols = [
            "ORGANIZATION",
            "NAICS",
        ...
        # build header with human-friendly timestamp in ET, no label
        now = datetime.now(ZoneInfo("America/New_York"))
        pulled = now.strftime("%B %d, %Y at %I:%M %p ET")
        header = (
            f"✅ **Forecast Bot Summary** ({len(blocks)} opportunities)<br/>"
            f"{pulled}<br/><br/>"
        )
